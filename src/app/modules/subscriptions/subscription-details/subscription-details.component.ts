...
export class SubscriptionDetailsComponent implements OnInit {
    ...
    if (this.printPermission) {
      ...
      if (this.sub.version == 'v4') {
        ...
        columns4 = [
          ...
          {
            title: `DISCOUNT ${
              this.sub?.codes ? '(' + this.sub?.codes?.percentage + '%)' : ''
            }`,
            dataKey: -this.sub?.discount_amount.toFixed(3),
          },
          {
  ...
  printAccountantTaxInvoice(sub: SubscriptionDetails) {
    if (this.printPermission) {
      ...
      doc.text(
        `${
          sub.location.landmark != '0' && sub.location.landmark != 'null'
            ? sub.location.landmark + ' - '
            : ''
        }${
          sub.location.property_number != '0' &&
          sub.location.property_number != 'null'
            ? sub.location.property_number
            : ''
        }`,
        85,
      ...
      doc.setFontSize(9);
      doc.text(
        sub.program_id == 60 ? 'FAST MEASUREMENT' : sub.subscriptions_note,
        47,
        90
      );
