...
export class ShowDislikeComponent implements OnInit, OnDestroy {
  ...
  getPermission() {
    this.printPermission =
      this._GuardService.getPermissionStatus('print_dislike');
    this.exportPermission =
      this._GuardService.getPermissionStatus('export_dislike');
    this.createPermission =
      this._GuardService.getPermissionStatus('create_dislike');
    this.updatePermission =
      this._GuardService.getPermissionStatus('update_dislike');
    this.downloadSamplePermission = this._GuardService.getPermissionStatus(
      'downloadSample_dislike'
    );
  ...
  exportAsPDF() {
    if (this.printPermission) {
      ...
      let filteredArray = this.allDislikes.filter((item: any) =>
        this.specificRows.includes(item.id)
      );
      filteredArray.length == 0 &&
        this.appliedFilters == null &&
        (filteredArray = this.allDislikes);
      filteredArray.length == 0 &&
        this.appliedFilters != null &&
        (filteredArray = this.dislikes);
  ...
  uploadFiles(form: FormGroup) {
    if (form.valid && this.uploadFilesPermission) {
      ...
      this._DislikeService
        .uploadDislikeRequestFiles(form.value)
        .subscribe((res) => {
