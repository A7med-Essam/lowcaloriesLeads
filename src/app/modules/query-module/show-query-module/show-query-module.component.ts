...
@Component({
  selector: 'app-show-query-module',
  templateUrl: './show-query-module.component.html',
  styleUrls: ['./show-query-module.component.scss'],
})
export class ShowQueryModuleComponent implements OnInit, OnDestroy {
  ...
  insertQuery(form: FormGroup) {
    if (form.valid) {
      this._SendBulkWhatsappService.addQueryService(form.value).subscribe({
        next: (res) => {
          if (res.status == 1) {
  ...
  deleteRow(id: number) {
    this._SendBulkWhatsappService.deleteQueryServices(id).subscribe((res) => {
      this.getQueries();
    });
  }
  page = 1;
  getQueries() {
    this._SendBulkWhatsappService
      .getQueryService(this.page, this.pagination.rows)
      .subscribe((res) => {
