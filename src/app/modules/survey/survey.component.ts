...
export class SurveyComponent implements OnInit {
  ...
  filter(
    ...
    filter3.value == undefined ? (filter3.value = []) : false;
    const FILTER = {
      survey_question_id: filter1.value,
      survey_answer_id: filter2.value,
      date: !filter3?.value[1] ? filter3?.value[0] : '',
      from: filter3?.value[1] ? filter3?.value[0] : '',
      to: filter3?.value[1] ? filter3?.value[1] : '',
    };
    this._SurveyService.getFilter(FILTER, page, paginate).subscribe((res) => {
  ...
  resetFilter(filter1: any, filter2: any, filter3: any) {
